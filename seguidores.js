(async () => {
  ...
  title.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
      ...
      <div>
        <div style="font-size:16px;">Hola <strong>@${username}</strong> 👋</div>
        <div style="font-size:12px;color:#666;">¿Quién te dejó de seguir en Instagram? 🕵️‍♂️💔</div>
        <div style="font-size:12px;color:#999;">*Solo muestra a los que tú sigues... y que ya no te siguen de vuelta 🙃</div>
      </div>
    </div>
  `;

  const filterInput = document.createElement('input');
  filterInput.placeholder = '🔍 Filtrar por nombre o usuario';
  ...
  verifiedToggle.appendChild(document.createTextNode('Incluir cuentas verificadas ✅'));
  ...
  status.innerText = '¿Estás listo?';
  status.style.marginTop = '10px';

  const footer = document.createElement('div');
  footer.innerText = 'Desarrollado por Karlo Buds · v060625.6';
  ...
  copyBtn.onclick = () => {
    ...
    alert('✅ Lista de usuarios copiada.');
  };

  const downloadBtn = document.createElement('button');
  downloadBtn.innerText = '⬇️ Descargar lista';
  ...
  stopBtn.onclick = () => {
    shouldStop = true;
    status.innerText = '⏹️ Escaneo detenido manualmente.';
    stopBtn.style.display = 'none';
        ...
        <a href="https://instagram.com/${node.username}" target="_blank" style="display:flex;align-items:center;text-decoration:none;color:#000;">
          ...
          <div>
            <div style="font-weight:bold;">
              ${node.full_name || '(Sin nombre)'} ${node.is_verified ? '✅' : ''}
            </div>
  ...
  async function scanFollowers() {
    ...
    if (!shouldStop) {
      status.innerText = `✅ Escaneo completo: ${nonFollowers.length} no te siguen`;
      startBtn.style.display = 'inline-block';
      stopBtn.style.display = 'none';
      copyBtn.style.display = 'inline-block';
      downloadBtn.style.display = 'inline-block';
